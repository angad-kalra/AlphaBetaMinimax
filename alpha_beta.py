...
def max_value(state, alpha, beta):
    ...
    for child in state.children():
        temp_state = min_value(child, alpha, beta)
        best_val = max(best_val, temp_state.payoff())
        state = temp_state
        if best_val >= beta :
            return temp_state
...
def min_value(state, alpha, beta):
    ...
    best_val = 9999
    for child in state.children():
        temp_state = max_value(child, alpha, beta)
        best_val = min(best_val, temp_state.payoff())
        state = temp_state
        if best_val <= alpha :
            return temp_state
